...
std::string encrypt_cbc(const std::string &plaintext, const byteVector &key, const byteVector &iv,
    ...
    int rc = EVP_EncryptInit_ex(ctx.get(),EVP_aes_256_cbc(), nullptr,key.data(),iv.data());
    if (rc != 1) {
...
void measure_cbc(unsigned trials, unsigned messageBlocks) {
    ...
    for(int i=0;i <trials;i++) {
        ...
        auto encryptedChangedMessage = encrypt_cbc(changedMessage,key,iv1,false);
    ...
    std::cout << "Average difference percentage in CBC mode: " << averageDifference << std::endl;
}
