...
std::string encrypt_ecb(const std::string &plaintext, const byteVector &key, bool enablePadding = true) {
    ...
    int rc = EVP_EncryptInit_ex(ctx.get(), EVP_camellia_256_ecb(), nullptr, key.data(), nullptr);
    if (rc != 1) {
...
void measure_ecb(unsigned trials, unsigned messageBlocks) {
    ...
    std::cout << "Average difference percentage in Camellia-ECB mode: " << averageDifference << std::endl;
}

int main() {
    EVP_add_cipher(EVP_camellia_256_ecb());
    measure_ecb(100000,3);
//    auto key = generateRandomBytes(32);
